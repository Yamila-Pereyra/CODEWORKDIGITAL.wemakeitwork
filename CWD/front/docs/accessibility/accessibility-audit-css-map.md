...
| --- | --- | --- | --- |
| Real production contact-form success and error behavior against the intended backend endpoint | Source proves there is no live region; this environment also compiled an undefined legacy contact endpoint because the legacy public API base URL was unset | The async network path and any production focus management after a real response were not verified against the intended backend contract | Browser test with a valid production-equivalent public API base URL, then keyboard and screen-reader review |
